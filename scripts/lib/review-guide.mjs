...
function renderActivityReviewWarning(activity, reviewById) {
  ...
  const details = flags
    ...
    .join("; ");
  return `${details}. Treat the frozen v0.1.0 interpretation below as a claim to review, not as independent confirmation.`;
}
...
export function buildDataReviewGuide(
  ...
  const lines = [
    ...
    "",
    `This is the human-readable review surface for calculator dataset **v${review.datasetVersion}**, evidence overlays **v${review.overlayVersion}**, reviewed **${review.reviewedOn}**. It exposes the existing values, source relationships, derivations, and limitations without changing the calculator, website, deployment, or immutable v0.1.0 release.`,
    "",
    ...
    "- [Activity](data/review/activity-evidence.schema.json) and [source](data/review/source-evidence.schema.json) review schemas — the machine-readable contracts for the overlays.",
    "- [Method records](data/methods/) — the calculator's bridge calculations and synthesis choices.",
    "- [Immutable v0.1.0 release](data/exports/v0.1.0/) — the portable, checksum-locked dataset currently published by the project.",
  ...
  for (const activity of activities) {
    ...
    lines.push(
      ...
      `- **Review-only additional sources:** ${additionalSources.length ? renderSourceLinks(additionalSources, sourceById, reviewById) : "None"}. These do not change the frozen activity's \`sourceIds\`.`,
      ...
      `- **Frozen v0.1.0 interpretation:** ${activity.note}`,
      ""
    );
  }
  ...
  lines.push(
    ...
    "",
    "The citation below is the reviewed citation. It may correct incomplete bibliographic metadata in the frozen v0.1.0 source record; those corrections are kept in this overlay so the release and website remain unchanged.",
    "",
  ...
  for (const source of sources) {
    ...
    lines.push(
      ...
      "",
      `- **Stable source ID:** \`${source.id}\` ([legacy v0.1.0 record](data/sources/${source.id}.json))`,
